...
module DataInput =
        ...
        static member Invoke(?class': string, ?color: Color, ?size: Size) =
            let classes = [
                ...
                if color.IsSome then
                    CssClass.ofColorVariant "checkbox" color.Value
                if size.IsSome then
                    CssClass.ofSize "checkbox" size.Value
                if class'.IsSome then
                    class'.Value
            ]
    ...
    type Radio =
        ...
        static member Invoke(?class': string, ?name: string, ?color: Color, ?size: Size) =
            let classes = [
                "radio"
                if color.IsSome then
                    CssClass.ofColorVariant "radio" color.Value
                if size.IsSome then
                    CssClass.ofSize "radio" size.Value
                if class'.IsSome then
                    class'.Value
            ]
        ...
        static member Invoke
            ...
            let classes = [
                ...
                if bordered = Some true then
                    "input-bordered"
                if ghost = Some true then
                    "input-ghost"
                if color.IsSome then
                    CssClass.ofColorVariant "input" color.Value
                if size.IsSome then
                    CssClass.ofSize "input" size.Value
                if class'.IsSome then
                    class'.Value
            ]
            ...
            if placeholder.IsSome then
                inp.placeholder <- placeholder.Value
        ...
        static member Invoke
            ...
            let classes = [
                ...
                if ghost = Some true then
                    "textarea-ghost"
                if color.IsSome then
                    CssClass.ofColorVariant "textarea" color.Value
                if size.IsSome then
                    CssClass.ofSize "textarea" size.Value
                if class'.IsSome then
                    class'.Value
            ]
            ...
            let ta =
                if placeholder.IsSome then
                    ta.placeholder <- placeholder.Value
                    ta
                else
                    ta
        ...
        static member Invoke(?class': string, ?bordered: bool, ?ghost: bool, ?color: Color, ?size: Size) =
            let classes = [
                ...
                if ghost = Some true then
                    "select-ghost"
                if color.IsSome then
                    CssClass.ofColorVariant "select" color.Value
                if size.IsSome then
                    CssClass.ofSize "select" size.Value
                if class'.IsSome then
                    class'.Value
            ]
    ...
    type Range =
        ...
        static member Invoke(?class': string, ?min: int, ?max: int, ?step: int, ?color: Color, ?size: Size) =
            let classes = [
                "range"
                if color.IsSome then
                    CssClass.ofColorVariant "range" color.Value
                if size.IsSome then
                    CssClass.ofSize "range" size.Value
                if class'.IsSome then
                    class'.Value
            ]
            ...
            let rng =
                if max.IsSome then
                    rng.max <- string max.Value
                    rng
                else
                    rng

            if step.IsSome then
                rng.step <- string step.Value
                rng
            else
                rng
    ...
    type Toggle =
        ...
        static member Invoke(?class': string, ?color: Color, ?size: Size) =
            let classes = [
                "toggle"
                if color.IsSome then
                    CssClass.ofColorVariant "toggle" color.Value
                if size.IsSome then
                    CssClass.ofSize "toggle" size.Value
                if class'.IsSome then
                    class'.Value
            ]
        ...
        static member Invoke(?class': string, ?bordered: bool, ?ghost: bool, ?color: Color, ?size: Size) =
            let classes = [
                ...
                if ghost = Some true then
                    "file-input-ghost"
                if color.IsSome then
                    CssClass.ofColorVariant "file-input" color.Value
                if size.IsSome then
                    CssClass.ofSize "file-input" size.Value
                if class'.IsSome then
                    class'.Value
            ]
    ...
    type Rating =
        ...
        static member Invoke(?class': string, ?size: Size, ?half: bool) =
            let classes = [
                "rating"
                if half = Some true then
                    "rating-half"
                if size.IsSome then
                    CssClass.ofSize "rating" size.Value
                if class'.IsSome then
                    class'.Value
            ]
    ...
    type Kbd =
        ...
        static member Invoke(?class': string, ?size: Size) =
            let classes = [
                "kbd"
                if size.IsSome then
                    CssClass.ofSize "kbd" size.Value
                if class'.IsSome then
                    class'.Value
            ]
