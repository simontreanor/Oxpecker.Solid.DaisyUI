...
module Actions =
        ...
        static member Invoke
            ...
            let classes = [
                ...
                if outline = Some true then
                    "btn-outline"
                if wide = Some true then
                    "btn-wide"
                if block = Some true then
                    "btn-block"
                if circle = Some true then
                    "btn-circle"
                if square = Some true then
                    "btn-square"
                if glass = Some true then
                    "btn-glass"
                if disabled = Some true then
                    "btn-disabled"
                if loading = Some true then
                    "loading"
                if active = Some true then
                    "btn-active"
                if color.IsSome then
                    CssClass.ofColorVariant "btn" color.Value
                if size.IsSome then
                    CssClass.ofSize "btn" size.Value
        ...
        static member Invoke
            ...
            let classes = [
                ...
                if hover = Some true then
                    "dropdown-hover"
                if ``open`` = Some true then
                    "dropdown-open"
                if top = Some true then
                    "dropdown-top"
                if bottom = Some true then
                    "dropdown-bottom"
                if left = Some true then
                    "dropdown-left"
                if right = Some true then
                    "dropdown-right"
                if ``end`` = Some true then
                    "dropdown-end"
                if class'.IsSome then
                    class'.Value
            ]
    ...
    type Swap =
        ...
        static member Invoke(?class': string, ?active: bool, ?rotate: bool, ?flip: bool) =
            let classes = [
                ...
                if active = Some true then
                    "swap-active"
                if rotate = Some true then
                    "swap-rotate"
                if flip = Some true then
                    "swap-flip"
                if class'.IsSome then
                    class'.Value
            ]
    ...
    type Collapse =
        ...
        static member Invoke(?class': string, ?arrow: bool, ?plus: bool, ?``open``: bool, ?close: bool) =
            let classes = [
                "collapse"
                if arrow = Some true then
                    "collapse-arrow"
                if plus = Some true then
                    "collapse-plus"
                if ``open`` = Some true then
                    "collapse-open"
                if close = Some true then
                    "collapse-close"
                if class'.IsSome then
                    class'.Value
            ]
