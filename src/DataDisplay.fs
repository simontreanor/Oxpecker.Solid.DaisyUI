...
module DataDisplay =
        ...
        static member Invoke(?class': string, ?color: ColorVariant, ?size: Size, ?outline: bool) =
            let classes = [
                ...
                if color.IsSome then
                    CssClass.ofColorVariant "badge" color.Value
                if size.IsSome then
                    CssClass.ofSize "badge" size.Value
        ...
        static member Invoke
            ...
            let classes = [
                ...
                if bordered = Some true then
                    "card-bordered"
                if imageFull = Some true then
                    "image-full"
                if compact = Some true then
                    "card-compact"
                if side = Some true then
                    "card-side"
                if color.IsSome then
    ...
    type Table =
        ...
        static member Invoke(?class': string, ?zebra: bool, ?pinRows: bool, ?pinCols: bool, ?size: Size) =
            let classes = [
                "table"
                if zebra = Some true then
                    "table-zebra"
                if pinRows = Some true then
                    "table-pin-rows"
                if pinCols = Some true then
                    "table-pin-cols"
                if size.IsSome then
                    CssClass.ofSize "table" size.Value
                if class'.IsSome then
                    class'.Value
            ]
    ...
    type ChatBubble =
        ...
        static member Invoke(?class': string, ?color: ColorVariant) =
            let classes = [
                "chat-bubble"
                if color.IsSome then
                    CssClass.ofColorVariant "chat-bubble" color.Value
                if class'.IsSome then
                    class'.Value
            ]
    ...
    type Mask =
        ...
        static member Invoke(?class': string, ?variant: string, ?half1: bool, ?half2: bool) =
            let classes = [
                ...
                | _ -> ""
                if half1 = Some true then
                    "mask-half-1"
                if half2 = Some true then
                    "mask-half-2"
                if class'.IsSome then
                    class'.Value
            ]
